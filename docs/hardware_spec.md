...
| **メイン基板要件** |
| :--- |
| バッテリーには 18650 電池を直列で 2 個使用 |
| マイコンには ESP32 32E を使用 |
| センサとして高度計（超音波センサ）、姿勢角計（9 軸センサ）、回転数計（フォトインタラプタ）、トリムボタン×3（タクトスイッチ）を搭載 ※基板発注後、高度計のみの使用に変更 |
| 表示器として LCD を搭載 |
| 設置場所はコックピット内 |
...
| バッテリーには 18650 電池を直列で 2 個使用 |
| マイコンには ESP32 32E を使用 |
| センサとして、機速計（差圧センサ）、舵角計×2（ポテンショメータ）、0 点ボタン×2（タクトスイッチ）、姿勢角計を搭載 （姿勢角計は当初メイン基板に搭載予定だったが、LCD 点灯時に値が取れなくなる不具合のため、TB に移動。I2C のため、発注した基板を変更する必要は無かった） |
| 設置場所は尾翼付近 |
